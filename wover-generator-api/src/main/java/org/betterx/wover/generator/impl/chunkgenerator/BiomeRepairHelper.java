...
class BiomeRepairHelper {
    ...
    private Map<ResourceKey<LevelStem>, ChunkGenerator> vanillaDimensions =
        null;

    public static TagKey<Biome> getBiomeTagForDimension(
    ...
    public Registry<LevelStem> repairBiomeSourceInAllDimensions(
        ...
        Map<ResourceKey<LevelStem>, ChunkGenerator> configuredDimensions =
            WorldGeneratorConfigImpl.loadWorldDimensions(
                registryAccess,
                WorldGeneratorConfigImpl.getPresetsNbt()
            );
        final Registry<Biome> biomes = registryAccess.lookupOrThrow(
            Registries.BIOME
        );
        ...
        for (Map.Entry<ResourceKey<LevelStem>, LevelStem> entry : originalSet) {
            ...
            final ChunkGenerator referenceGenerator = configuredDimensions.get(
                key
            );

            if (
                referenceGenerator instanceof
                    EnforceableChunkGenerator<?> enforcer
            ) {
                final ChunkGenerator loadedChunkGenerator =
                    loadedStem.generator();
                ...
                registerAllBiomesFromVanillaDimension(
                    registryAccess,
                    biomes,
                    key
                );
                ...
                if (enforcer.togetherShouldRepair(loadedChunkGenerator)) {
                    dimensionRegistry =
                        enforcer.enforceGeneratorInWorldGenSettings(
                            registryAccess,
                            key,
                    ...
                    loadedChunkGenerator.getBiomeSource() instanceof
                        BiomeSourceWithConfig lodedSource
                ) {
                    if (
                        referenceGenerator.getBiomeSource() instanceof
                            BiomeSourceWithConfig refSource
                    ) {
                        if (
                            !refSource
                                .getBiomeSourceConfig()
                                .sameConfig(lodedSource.getBiomeSourceConfig())
                        ) {
                            lodedSource.setBiomeSourceConfig(
                                refSource.getBiomeSourceConfig()
                            );
                        }
                    }
                }
            ...
            if (!didRepair) {
                if (
                    loadedStem.generator().getBiomeSource() instanceof
                        ReloadableBiomeSource reload
                ) {
                    reload.reloadBiomes();
                }
            }
    ...
    private static void copyWorldPresetReference(
        ...
        for (Map.Entry<
            ...
            LevelStem
        > loadedDimension : dimensionRegistry.entrySet()) {
            final ChunkGenerator referenceGenerator = configuredDimensions.get(
                loadedDimension.getKey()
            );

            if (
                referenceGenerator instanceof ConfiguredChunkGenerator refCfg &&
                loadedDimension.getValue().generator() instanceof
                    ConfiguredChunkGenerator loadedCfg
            ) {
                if (loadedCfg.wover_getConfiguredWorldPreset() == null) {
                    loadedCfg.wover_setConfiguredWorldPreset(
                        refCfg.wover_getConfiguredWorldPreset()
                    );
                }
            }
        }
    ...
    private void registerAllBiomesFromFabric(Registry<Biome> biomes) {
        final Stopwatch sw = Stopwatch.createStarted();
        int biomesAdded = 0;

        final BiomeTagModificationWorker biomeTagWorker =
            new BiomeTagModificationWorker();
        for (Map.Entry<ResourceKey<Biome>, Biome> e : biomes.entrySet()) {
            ...
            if (NetherBiomes.canGenerateInNether(e.getKey())) {
                ...
                TheEndBiomesHelper.canGenerateAsMainIslandBiome(e.getKey())
            ) {
                tag = CommonBiomeTags.IS_END_CENTER;
            } else if (
                TheEndBiomesHelper.canGenerateAsHighlandsBiome(e.getKey())
            ) {
                ...
                TheEndBiomesHelper.canGenerateAsSmallIslandsBiome(e.getKey())
            ) {
                tag = CommonBiomeTags.IS_SMALL_END_ISLAND;
            } else if (
                TheEndBiomesHelper.canGenerateAsEndMidlands(e.getKey())
            ) {
                tag = CommonBiomeTags.IS_END_MIDLAND;
            }

            if (tag != null) {
                final Holder.Reference<Biome> holder = biomes.getOrThrow(
                    e.getKey()
                );
                if (!holder.is(tag)) {
                    biomeTagWorker.addBiomeToTag(
                        tag,
                        biomes,
                        e.getKey(),
                        holder
        ...
        if (biomesAdded > 0) {
            LibWoverWorldGenerator.C.log.info(
                "Added Tags for {} fabric biomes in {}",
                biomesAdded,
                sw
            );
        }
    ...
    private void registerAllBiomesFromVanillaDimension(
        ...
        if (tag != null) {
            ...
            final ChunkGenerator vanillaDim = vanillaDimensions.getOrDefault(
                key,
                null
            );
            if (vanillaDim != null && vanillaDim.getBiomeSource() != null) {
                for (Holder<Biome> biomeHolder : vanillaDim
                    .getBiomeSource()
                    .possibleBiomes()) {
                    if (
                        biomeHolder.unwrapKey().isPresent() &&
                        !biomeHolder.is(tag)
                    ) {
                        biomeTagWorker.addBiomeToTag(
                            tag,
                            biomes,
                            biomeHolder.unwrapKey().orElseThrow(),
                            biomeHolder
                        );
                        biomesAdded++;
                    }
