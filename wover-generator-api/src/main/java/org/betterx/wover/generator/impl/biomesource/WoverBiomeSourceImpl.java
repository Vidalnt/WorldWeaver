...
public class WoverBiomeSourceImpl {
        ...
        return namespaces
            ...
            .map(
                n ->
                    n +
                    "(" +
                    namespaces.stream().filter(n::equals).count() +
                    ")"
            )
    ...
    public static @Nullable Set<Holder<Biome>> populateBiomePickers(
        ...
        if (access == null) {
            ...
            if (access != null) {
                LibWoverWorldGenerator.C.log.verbose(
                    "Registries were not finalized before populating BiomePickers!"
                );
            } else {
                if (!ModCore.isDatagen()) {
                    LibWoverWorldGenerator.C.log.verbose(
                        "Unable to build Biome List yet"
                    );
                }
                return null;
            }
        ...
        final Registry<BiomeData> biomeData = access.lookupOrThrow(
            BiomeDataRegistry.BIOME_DATA_REGISTRY
        );

        for (WoverBiomeSource.TagToPicker mapper : pickers) {
            final Optional<HolderSet.Named<Biome>> optionalTag = biomes.get(
                mapper.tag()
            );
            if (optionalTag.isPresent()) {
                final HolderSet.Named<Biome> tag = optionalTag.get();
                final Set<Identifier> excluded =
                    BiomeSourceManagerImpl.getExcludedBiomes(tag.key());
