...
public class ChunkGeneratorManagerImpl {
    ...
    private static final Identifier LEGACY_ID =
        LegacyHelper.BCLIB_CORE.convertNamespace(WoverChunkGenerator.ID);
     ...
     */
    @ApiStatus.Internal
    public static Map<String, Supplier<TypeTemplate>> addGeneratorDSL(
    ...
    public static void onWorldReCreate(
        ...
        final var configuredPreset =
            WorldGeneratorConfigImpl.getPresetsNbtFromFolder(storage);
        final var dimensions = WorldGeneratorConfigImpl.loadWorldDimensions(
            WorldState.allStageRegistryAccess(),
            configuredPreset
        );

        for (var dimEntry : context
            .selectedDimensions()
            .dimensions()
            .entrySet()) {
            final var refDim = dimensions.get(dimEntry.getKey());
            if (
                refDim instanceof ConfiguredChunkGenerator refGen &&
                refGen.wover_getConfiguredWorldPreset() != null &&
                dimEntry.getValue().generator() instanceof
                    ConfiguredChunkGenerator loadGen &&
                loadGen.wover_getConfiguredWorldPreset() == null
            ) {
                loadGen.wover_setConfiguredWorldPreset(
                    refGen.wover_getConfiguredWorldPreset()
                );
            }
        }
    }
    ...
    public static void register(
        ...
        if (GENERATOR_IDS.contains(idString)) {
            throw new IllegalStateException(
                "Duplicate generator id: " + idString
            );
        }
    ...
    public static String enumerateFeatureNamespaces(
            ...
            if (supplier != null) {
                ...
                try {
                            ...
                            if (features != null) {
                                for (PlacedFeature feature : features.features()) {
                                    if (feature != null) {
                                        ...
                                        if (
                                            WorldState.registryAccess() != null
                                        ) {
                    ...
                    LibWoverWorldGenerator.C.log.warn(
                        "Failed to enumerate feature namespaces",
                        e
                    );
                }
                return namespaces
    ...
    public static String printGeneratorInfo(
        ...
        if (generator instanceof ChunkGeneratorAccessor) {
            sb
                .append("\n    features   = ")
                .append(enumerateFeatureNamespaces(generator));
        }
    ...
    public static void printDimensionInfo(WorldDimensions dimensionRegistry) {
        ...
        printDimensionInfo(
            "World Dimensions",
            dimensionRegistry.dimensions().entrySet()
        );
    }
    ...
    public static void printDimensionInfo(
        String title,
        WorldDimensions dimensionRegistry
    ) {
    ...
    public static void printDimensionInfo(
        String title,
        Set<Map.Entry<ResourceKey<LevelStem>, LevelStem>> levels
    ) {
