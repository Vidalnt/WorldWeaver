...
public class WoverChunkGeneratorImpl {
    ...
    > LEGACY_AMPLIFIED_NETHER = ResourceKey.create(
        Registries.NOISE_SETTINGS,
        LegacyHelper.BCLIB_CORE.convertNamespace(
            WoverChunkGenerator.AMPLIFIED_NETHER.identifier()
        )
    );

    @ApiStatus.Internal
    public static void initialize() {
        WorldLifecycle.MINECRAFT_SERVER_READY.subscribe(
            WoverChunkGeneratorImpl::restoreInitialBiomeSourceInAllDimensions
        );
        WorldLifecycle.ON_DIMENSION_LOAD.subscribe(
            WoverChunkGeneratorImpl::repairBiomeSourceInAllDimensions
        );
        WorldLifecycle.BEFORE_CREATING_LEVELS.subscribe(
            WoverChunkGeneratorImpl::printInfo,
            -1000
        );
    }
    ...
    private static void printInfo(
        ...
        if (WorldState.registryAccess() != null) {
            final Registry<LevelStem> dimensionsRegistry =
                WorldState.registryAccess().lookupOrThrow(
    ...
    private static void restoreInitialBiomeSourceInAllDimensions(
        ...
        for (var entry : WorldState.registryAccess()
            ...
            if (
                stem.generator() instanceof RestorableBiomeSource<?> generator
            ) {
    ...
    public static Registry<LevelStem> replaceGenerator(
        ...
        > entry : dimensionRegistry) {
            ...
            if (
                dimensionKey.identifier().equals(resourceKey.identifier())
            ) continue;
