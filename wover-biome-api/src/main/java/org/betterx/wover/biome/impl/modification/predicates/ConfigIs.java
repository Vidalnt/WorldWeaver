...
public record ConfigIs(
    Identifier configFile,
    String path,
    String key,
    String testValue
) implements BiomePredicate {
    public static final MapCodec<ConfigIs> DIRECT_CODEC =
        RecordCodecBuilder.mapCodec(instance ->
            instance
                .group(
                    Identifier.CODEC.fieldOf("config_file").forGetter(
                        ConfigIs::configFile
                    ),
    ...
    public static final KeyDispatchDataCodec<ConfigIs> CODEC =
    ...
    public static <T, R extends AbstractConfig<?>.Value<T, R>> ConfigIs of(
        ...
        if (value.getParentFile() == null) {
            throw new IllegalArgumentException(
                "Value " + value + " must have a parent file."
            );
        }
    ...
    public boolean test(Context ctx) {
        ...
        final AbstractConfig<?>.Value<
            ?,
            ? extends AbstractConfig<?>.Value<?, ?>
        > value = config.getValue(path, key);
        if (value == null) {
            LibWoverCore.C.log.verboseWarning(
                "Config value %s.%s not found in %s",
                path,
                key,
                configFile
            );
