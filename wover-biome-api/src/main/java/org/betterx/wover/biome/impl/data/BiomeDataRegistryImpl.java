...
public class BiomeDataRegistryImpl {
    ...
    public static final EventImpl<
        OnBootstrapRegistry<BiomeData>
    > BOOTSTRAP_BIOME_DATA_REGISTRY = new EventImpl<>(
        "BOOTSTRAP_BIOME_DATA_REGISTRY"
    );
    ...
    private static final CustomRegistryData.DataKey<
        Map<ResourceKey<BiomeData>, BiomeData>
    > TEMP_BIOME_DATA = CustomRegistryData.createKey(
        LibWoverBiome.C.id("temp_biome_data")
    );
    ...
    public static @Nullable BiomeData getFromRegistryOrTemp(
        ...
        final Registry<BiomeData> registry =
    ...
    public static @Nullable BiomeData getFromRegistryOrTemp(
        ...
        if (registry instanceof CustomRegistryData tempRegistryData) {
            Map<ResourceKey<BiomeData>, BiomeData> customData =
                tempRegistryData.wover_computeDataIfAbsent(TEMP_BIOME_DATA, k ->
                    new HashMap<>()
                );

            return customData.computeIfAbsent(dataKey, k ->
                BiomeData.tempOf(key)
            );
        }
