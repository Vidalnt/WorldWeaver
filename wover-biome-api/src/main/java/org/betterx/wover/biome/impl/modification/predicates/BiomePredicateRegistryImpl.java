...
public class BiomePredicateRegistryImpl {
    ...
    public static final Registry<
        MapCodec<? extends BiomePredicate>
    > BIOME_PREDICATES = BuiltInRegistryManager.createRegistry(
    ...
    private static MapCodec<? extends BiomePredicate> onBootstrap(
        Registry<MapCodec<? extends BiomePredicate>> registry
    ) {
        ...
        register(
            registry,
            LibWoverBiome.C.id("in_dimension"),
            InDimension.CODEC
        );
        register(
            registry,
            LibWoverBiome.C.id("is_namespace"),
            IsNamespace.CODEC
        );
        register(
            registry,
            LibWoverBiome.C.id("location_path_contains"),
            LocationPathContains.CODEC
        );
        register(registry, LibWoverBiome.C.id("spawns"), Spawns.CODEC);
        register(
            registry,
            LibWoverBiome.C.id("has_structure"),
            HasStructure.CODEC
        );
        register(
            registry,
            LibWoverBiome.C.id("has_placed_feature"),
            HasPlacedFeature.CODEC
        );
        register(
            registry,
            LibWoverBiome.C.id("has_configured_feature"),
            HasConfiguredFeature.CODEC
        );
