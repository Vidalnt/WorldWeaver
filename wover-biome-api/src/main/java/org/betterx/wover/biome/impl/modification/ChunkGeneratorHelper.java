...
public class ChunkGeneratorHelper {
    ...
    public static void rebuildFeaturesPerStep(
        ChunkGenerator generator,
        BiomeSource biomeSource
    ) {
        if (generator instanceof ChunkGeneratorAccessor acc) {
            Function<Holder<Biome>, BiomeGenerationSettings> function = (Holder<
                Biome
            > biomeHolder) -> biomeHolder.value().getGenerationSettings();
            acc.wover_setFeaturesPerStep(
                Suppliers.memoize(() -> {
                    try {
                        ...
                        LibWoverBiome.C.LOG.error(
                            "Failed to rebuild features per step",
                            e
                        );
                        for (Holder<
                            Biome
                        > biome : biomeSource.possibleBiomes()) {
                            var loc = biome
                                .unwrapKey()
                                .orElseThrow()
                                .identifier()
