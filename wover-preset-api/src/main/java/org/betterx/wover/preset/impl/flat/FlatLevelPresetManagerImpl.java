...
public class FlatLevelPresetManagerImpl {
    ...
    public static final EventImpl<
        OnBootstrapFlatLevelPresets
    > BOOTSTRAP_FLAT_LEVEL_PRESETS = new EventImpl<>(
        "BOOTSTRAP_FLAT_LEVEL_PRESETS"
    );

    public static ResourceKey<FlatLevelGeneratorPreset> createKey(
        Identifier loc
    ) {
    ...
    private static void onBootstrap(
        BootstrapContext<FlatLevelGeneratorPreset> context
    ) {
        FlatLevelPresetBootstrapContext ctx =
            new FlatLevelPresetBootstrapContext(context);
    ...
    public static void register(
        ...
        final HolderGetter<StructureSet> structureSets = ctx.lookup(
            Registries.STRUCTURE_SET
        );
        Objects.requireNonNull(structureSets);

        final HolderGetter<PlacedFeature> placedFeatures = ctx.lookup(
        ...
        ctx.register(
            presetKey,
            new FlatLevelGeneratorPreset(
                icon.asItem().builtInRegistryHolder(),
