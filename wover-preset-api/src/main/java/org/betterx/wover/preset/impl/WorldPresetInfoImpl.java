...
public class WorldPresetInfoImpl implements WorldPresetInfo {
    ...
    public static final Codec<WorldPresetInfo> CODEC =
            ...
            instance
                .group(
                    ...
                    ResourceKey.codec(Registries.WORLD_PRESET)
                        ...
                        .forGetter(o ->
                            Optional.ofNullable(o.overworldPreset())
                        ),
                    ResourceKey.codec(Registries.WORLD_PRESET)
    ...
    @Override
    public @Nullable ResourceKey<WorldPreset> getPresetOverride(
        ResourceKey<LevelStem> key
    ) {
        if (
            key.identifier().equals(LevelStem.OVERWORLD.identifier())
        ) return overworldPreset();
        if (
            key.identifier().equals(LevelStem.NETHER.identifier())
        ) return netherPreset();
        if (
            key.identifier().equals(LevelStem.END.identifier())
        ) return endPreset();
        return null;
    }
    ...
    public @Nullable ResourceKey<WorldPreset> getPresetOverrideRecursive(
        ...
        ResourceKey<WorldPreset> overrideKey = this.getPresetOverride(
            forDimension
        );

        while (overrideKey != null && count > 0) {
            configuredKey = overrideKey;
            overrideKey = WorldPresetInfoRegistry.getFor(
                configuredKey
