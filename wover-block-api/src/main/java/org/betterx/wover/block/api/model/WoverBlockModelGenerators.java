...
public class WoverBlockModelGenerators {
    ...
    public static final Identifier CROSS = Identifier.withDefaultNamespace(
        "block/cross"
    );
    public static final Identifier CUBE = Identifier.withDefaultNamespace(
        "block/cube"
    );
    public static final Identifier CUBE_ALL = Identifier.withDefaultNamespace(
        "block/cube_all"
    );
    public static final Identifier COMPOSTER = LibWoverBlock.C.id(
        "block/composter"
    );
    private static final Identifier LADDER = Identifier.withDefaultNamespace(
        "block/ladder"
    );
    ...
    public void createObsidianVariants(
        WoverBlockModelGenerators generators,
        Block obsidianBlock
    ) {
        var model = generators.getTextureModels(
            obsidianBlock,
            TexturedModel.CUBE.get(obsidianBlock)
        );
        var template = model.getTemplate();
        var modelLocation = template.create(
            obsidianBlock,
            model.getMapping(),
        ...
        for (VariantMutator rotation : rotations) {
            for (VariantMutator rotationY : rotations) {
                ...
                if (rotation != NOP) variants[idx] = rotation.apply(
                    variants[idx]
                );

                if (rotationY != NOP) variants[idx] = rotationY.apply(
                    variants[idx]
                );
    ...
    public void delegateItemModel(Block block) {
        this.vanillaGenerator.registerSimpleItemModel(
            block,
    ...
    public TexturedModel getTextureModels(
        Block block,
        TexturedModel defaultModel
    ) {
        return BlockModelGenerators.TEXTURED_MODELS.getOrDefault(
            block,
            defaultModel
        );
    }
    ...
    public Builder modelFor(Block block) {
        final TexturedModel texturedModel = this.getTextureModels(
            block,
    ...
    public Builder modelFor(
        Block block,
        TextureMapping textureMappingOverride
    ) {
        final TexturedModel texturedModel = this.getTextureModels(
            block,
            TexturedModel.CUBE.get(block)
    ...
    public void createBookshelf(Block shelf, Block planks) {
        ...
        acceptBlockState(
            vanillaGenerator.createSimpleBlock(
                shelf,
    ...
    private static final PropertyDispatch<
        VariantMutator
    > ROTATION_HORIZONTAL_FACING = PropertyDispatch.modify(
    ...
    public void createLadder(Block ladderBlock) {
        ...
        var mapping = new TextureMapping()
            .put(
                TextureSlot.PARTICLE,
                TextureMapping.getBlockTexture(ladderBlock)
            )
            .put(
                TextureSlot.TEXTURE,
                TextureMapping.getBlockTexture(ladderBlock)
        ...
        acceptBlockState(
            MultiVariantGenerator.dispatch(
                ladderBlock,
                BlockModelGenerators.plainVariant(ladderModel)
    ...
    public void createBars(Block barsBlock) {
        ...
        MultiVariant postEndsVariant = plainVariant(
            barsModel.withSuffix("_post_ends")
        );
        ...
        MultiVariant altCapVariant = plainVariant(
            barsModel.withSuffix("_cap_alt")
        );
        MultiVariant altSideVariant = plainVariant(
            barsModel.withSuffix("_side_alt")
        );
    ...
    private final Map<Identifier, Identifier> PARTICLE_ONLY_MODELS =
        Maps.newHashMap();

    public Identifier particleOnlyModel(Block block) {
        var name = ModelLocationUtils.getModelLocation(block).withSuffix(
            "_particles"
        );
        if (name.getNamespace().equals("minecraft")) name = LibWoverBlock.C.mk(
            name.getPath()
        );
        ...
        if (
            ...
            textureName.getPath().endsWith("_log")
        ) textureName = textureName.withSuffix("_side");

        Identifier finalName = name;
        Identifier finalTextureName = textureName;
        return PARTICLE_ONLY_MODELS.computeIfAbsent(name, n ->
    ...
    public void createBarrel(Block barrelBlock) {
        Identifier resourceLocation = TextureMapping.getBlockTexture(
            barrelBlock,
            "_top_open"
        );
        ...
        MultiVariant openVariant = BlockModelGenerators.plainVariant(
            TexturedModel.CUBE_TOP_BOTTOM.get(barrelBlock)
        ...
        acceptBlockState(
            MultiVariantGenerator.dispatch(barrelBlock)
                .with(
    ...
    public void createComposter(Block composterBlock) {
        var mapping = new TextureMapping()
            .put(
                TextureSlot.SIDE,
                TextureMapping.getBlockTexture(composterBlock, "_side")
            )
            .put(
                TextureSlot.TOP,
                TextureMapping.getBlockTexture(composterBlock, "_top")
            )
            .put(
                TextureSlot.BOTTOM,
                TextureMapping.getBlockTexture(composterBlock, "_bottom")
            );
        var location = COMPOSTER_MODEL.create(
            composterBlock,
            mapping,
            vanillaGenerator.modelOutput
        );
        acceptBlockState(
            MultiPartGenerator.multiPart(composterBlock)
                .with(BlockModelGenerators.plainVariant(location))
                .with(
                    BlockModelGenerators.condition().term(
    ...
    public void createBlockTopSideBottom(
        Block bottomBlock,
        Block coverBlock,
        ...
        var mapping = new TextureMapping()
            .put(
                TextureSlot.SIDE,
                TextureMapping.getBlockTexture(coverBlock, "_side")
            )
            .put(
                TextureSlot.TOP,
                TextureMapping.getBlockTexture(coverBlock, "_top")
            )
            .put(
                TextureSlot.BOTTOM,
                TextureMapping.getBlockTexture(bottomBlock)
            );
        var location = ModelTemplates.CUBE_BOTTOM_TOP.create(
            coverBlock,
            mapping,
            vanillaGenerator.modelOutput
        );

        if (withVariants) acceptBlockState(
            randomTopModelVariant(coverBlock, location)
        );
        else acceptBlockState(
            BlockModelGenerators.createSimpleBlock(
                coverBlock,
    ...
    public void createCubeModel(Block block) {
        ...
        final TextureMapping mapping = this.getTextureModels(
            block,
            model
        ).getMapping();
        final var location = model
            .getTemplate()
            .create(block, mapping, vanillaGenerator.modelOutput);
        acceptBlockState(
            BlockModelGenerators.createSimpleBlock(
                block,
                BlockModelGenerators.plainVariant(location)
    ...
    public void createPressurePlate(
        Block plateBlock,
        Identifier textureLocation
    ) {
        createPressurePlate(
            plateBlock,
            new TextureMapping().put(TextureSlot.TEXTURE, textureLocation)
        );
    }
    ...
    public void createPressurePlate(Block materialBlock, Block plateBlock) {
        createPressurePlate(
            plateBlock,
            this.getTextureModels(
                plateBlock,
                TexturedModel.CUBE.get(materialBlock)
            ).getMapping()
        );
    }
    ...
    public void createSlab(
        ...
        final var fullBlockLocation = ModelLocationUtils.getModelLocation(
            baseBlock
        );
        final List<Identifier> locations = Stream.of(
    ...
    public void createLog(Block logBlock, TextureMapping mapping) {
    ...
    public void createLog(
        ...
        if (alternatives.length > 0 || mirroredAlternative) {
            ...
            for (TextureMapping alt : alternatives)
                addModels.accept(alt, count++);
            ...
            delegateItemModel(
                logBlock,
                variantList.unwrap().getFirst().value().modelLocation()
            );
        } else {
            Identifier first = ModelTemplates.CUBE_COLUMN.create(
                logBlock,
                mapping,
                vanillaGenerator.modelOutput
            );
    ...
    public void createRotatedPillar(
        ...
        if (alternatives.length > 0 || mirroredAlternative) {
            ...
            final var model = ModelTemplates.CUBE_COLUMN.create(
                pillarBlock,
                mapping,
                vanillaGenerator.modelOutput
            );
    ...
    public final void createItemModel(
        Block block,
    ...
    public static MultiVariantGenerator randomTopModelVariant(
        Block block,
        Identifier model
    ) {
        return MultiVariantGenerator.dispatch(
            ...
            BlockModelGenerators.variants(
                ...
                BlockModelGenerators.Y_ROT_270.apply(
    ...
    public class Builder {
        ...
        public Builder createCustomFence(Block fenceBlock) {
            final TextureMapping particles = TextureMapping.customParticle(
                fenceBlock
            );
        ...
        private Builder createFullBlockVariant(Block block) {
            final TexturedModel texturedModel = getTextureModels(
                block,
                TexturedModel.CUBE.get(block)
            );
            final Identifier resourceLocation = texturedModel.create(
                block,
                vanillaGenerator.modelOutput
            );
        ...
        public Builder createSlab(Block slabBlock) {
            if (this.fullBlockLocation == null) {
                throw new IllegalStateException(
                    "Please call createFullBlock before calling createSlab"
                );
            } else {
                final List<Identifier> locations = Stream.of(
                    ModelTemplates.SLAB_BOTTOM,
                    ...
                    .map(template ->
                        this.computeModelIfAbsent(template, slabBlock)
                    )
                    .toList();
        ...
        private Identifier computeModelIfAbsent(
            ModelTemplate modelTemplate,
            Block block
        ) {
            return this.models.computeIfAbsent(modelTemplate, m ->
                m.create(block, this.mapping, vanillaGenerator.modelOutput)
            );
        }
