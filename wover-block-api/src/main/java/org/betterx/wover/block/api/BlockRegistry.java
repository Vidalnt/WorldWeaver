...
public abstract class BlockRegistry {
    ...
    public @NotNull ResourceKey<Item> blockItemKey(
    ...
    public <B extends Block> DefaultBlockDefinition<
        B
    > defineDefaultBlockWithProps(
        String blockName,
        Function<BlockBehaviour.Properties, B> blockFactory
    ) {
        return new DefaultBlockDefinition<>(this, blockName, def ->
            blockFactory.apply(def.properties)
        );
    }
        ...
        if (register(key, block, tags)) {
            ...
            if (block instanceof CustomBlockItemProvider provider) {
                item = provider.getCustomBlockItem(
                    itemKey.identifier(),
                    defaultBlockItemSettings().setId(
                        ResourceKey.create(
                            BuiltInRegistries.ITEM.key(),
                            itemKey.identifier()
                        )
                ...
                item = WoverBlockItemImpl.create(
                    block,
                    defaultBlockItemSettings().setId(itemKey)
                );
            }

            if (itemTags == null) registerBlockItem(itemKey, item);
            else registerBlockItem(itemKey, item, itemTags);
        }
    }

    @SafeVarargs
    public final <T extends Block> T register(
        String path,
        T block,
        TagKey<Block>... tags
    ) {
    ...
    private void _registerBlockOnly(
        ...
        ) datagenTags.put(block, tags);
    }

    @SafeVarargs
    public final <T extends Block> T registerBlockOnly(
        String path,
        T block,
        TagKey<Block>... tags
    ) {
    ...
    public void bootstrapBlockTags(TagBootstrapContext<Block> ctx) {
        ...
        blocks
            ...
            .forEach(b ->
                ((BlockTagProvider) b.getValue()).registerBlockTags(
                    b.getKey().identifier(),
                    ctx
                )
            );
    ...
    public void bootstrapBlockLoot(
        ...
        blocks
            ...
            .forEach(b -> {
                ...
                var builder = (
                    (BlockLootProvider) b.getValue()
                ).registerBlockLoot(b.getKey().identifier(), provider, key);

                if (builder != null) biConsumer.accept(key, builder);
            });
    }
    ...
    public static void registerAsFlammable(Block block, int burn, int spread) {
        if (
