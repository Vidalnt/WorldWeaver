...
public class UpdatesScreen extends WoverLayoutScreen {
    ...
    static final Identifier UPDATE_LOGO_LOCATION = LibWoverUi.C.mk(
        "icon_updater.png"
    );
    ...
    public UpdatesScreen(@NotNull Runnable onClose) {
        super(
            onClose,
            Component.translatable("wover.updates.title"),
            10,
            10,
            10
        );
    ...
    public static void showUpdateUI() {
        ...
        Minecraft.getInstance().setScreen(
            new UpdatesScreen(Minecraft.getInstance().screen)
        );
    }
    ...
    protected LayoutComponent<?, ?> initContent() {
        ...
        rows
            .addMultilineText(
                fill(),
                fit(),
                Component.translatable("wover.updates.description")
            )
            .centerHorizontal();
        ...
        VersionChecker.forEachUpdate((mod, cur, updated) -> {
            ...
            HorizontalStack row = rows
                .addRow(fixed(320), fit())
                .centerHorizontal();
            if (icon != null) {
            ...
            if (nfo != null) {
                row
                    .addText(
                        fit(),
                        fit(),
                        Component.literal(nfo.getMetadata().getName())
                    )
                    .setColor(ColorHelper.WHITE);
            } else {
                row
                    .addText(fit(), fit(), Component.literal(mod))
                    .setColor(ColorHelper.WHITE);
            }
            row.addSpacer(4);
            ...
            row
                .addText(fit(), fit(), Component.literal(updated))
                .setColor(ColorHelper.GREEN);
            row.addFiller();
            ...
            if (
                ...
                CustomValue.CvObject downloadLinks = nfo
                    .getMetadata()
                    .getCustomValue("wover")
                    .getAsObject()
                    .get("downloads")
            ...
            if (
                !createdDownloadLink &&
                nfo != null &&
                nfo.getMetadata().getContact().get("homepage").isPresent()
            ) {
                row
                    .addButton(
                        fit(),
                        fit(),
                        Component.translatable("wover.updates.download_link")
                    )
                    .onPress(bt -> {
                        this.openLink(
