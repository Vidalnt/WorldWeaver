...
public class DatapackRegistryBuilderImpl {
    ...
    private static final PriorityLinkedList<Entry<?>> REGISTRIES =
        new PriorityLinkedList<>();

    private record Entry<T>(
        ResourceKey<? extends Registry<T>> key,
        @Nullable Codec<T> elementCodec,
        ...
        public BootstrapContext<T> getContext(
            ...
            return DatapackRegistryBuilder.makeContext(
                registryInfoLookup,
                registry
            );
        }
    ...
    public static <T> void register(
        ...
        LibWoverCore.C.log.debug(
            "Adding dynamic registry bootstrap: " + key.identifier()
        );
        REGISTRIES.add(
            new Entry<>(key, null, bootstrap),
            Math.max(MAX_READONLY_PRIORITY + 1, priority)
        );
    }
    ...
    public static <T> void registerReadOnly(
        ...
        register(
            key,
            bootstrap,
            Math.min(MAX_READONLY_PRIORITY, Integer.MIN_VALUE + priority)
        );
    }
    ...
    public static <T> void register(
        ...
        LibWoverCore.C.log.debug(
            "Adding dynamic registry: " + key.identifier()
        );
        REGISTRIES.add(new Entry<>(key, elementCodec, bootstrap), priority);
    }

    public static void forEach(
        BiConsumer<ResourceKey<? extends Registry<?>>, Codec<?>> consumer
    ) {
        initEntrypoints();
    ...
    private static void initEntrypoints() {
        ...
        LibWoverCore.C.LOG.verbose(
            "Processing wover.datapack.registry Entrypoints"
        );
        FabricLoader.getInstance()
            .getEntrypoints(
                "wover.datapack.registry",
                DatapackRegistryEntrypoint.class
            )
            .forEach(entrypoint -> {
                LibWoverCore.C.LOG.verbose(
    ...
    public static <E> void bootstrap(
        ...
        REGISTRIES.forEach(entry -> {
            if (entry.key.equals(resourceKey)) {
                LibWoverCore.C.LOG.debug(
                    "Calling custom Registry Bootstrap on {}",
                    resourceKey.identifier()
                );
                entry.bootstrap.accept(
                    entry.getContext(
                        registryInfoLookup,
    ...
    public static void bootstrap(
        BiConsumer<
            ...
            RegistrySetBuilder.RegistryBootstrap<? extends Object>
        > consumer
    ) {
        LibWoverCore.C.LOG.verboseWarning(
            "DID NOT bootstrap VanillaRegistries."
        );
        //        initEntrypoints();
