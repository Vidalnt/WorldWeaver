...
public class SurfaceRuleUtil {
    ...
    private static List<SurfaceRules.RuleSource> getRulesForBiome(
        ResourceKey<Biome> biomeKey
    ) {
        Registry<AssignedSurfaceRule> registry = null;
        if (WorldState.registryAccess() != null) registry =
            WorldState.registryAccess()
                .lookup(SurfaceRuleRegistry.SURFACE_RULES_REGISTRY)
                .orElse(null);
        ...
        var list = registry
            ...
            .filter(
                a ->
                    a != null &&
                    a.biomeID != null &&
                    a.biomeID.equals(biomeKey.identifier())
            )
    ...
    private static List<SurfaceRules.RuleSource> getRulesForBiomes(
        List<Optional<ResourceKey<Biome>>> biomes
    ) {
        List<ResourceKey<Biome>> biomeIDs = biomes
            .stream()
    ...
    private static SurfaceRules.RuleSource mergeSurfaceRules(
        ...
        if (org instanceof SurfaceRules.SequenceRuleSource sequenceRule) {
            List<SurfaceRules.RuleSource> existingSequence =
            ...
            if (dimensionKey.equals(LevelStem.NETHER)) {
                final List<SurfaceRules.RuleSource> combined = new ArrayList<>(
                    existingSequence.size() + additionalRules.size()
                );
                for (SurfaceRules.RuleSource rule : existingSequence) {
                    if (
                        rule instanceof SurfaceRules.TestRuleSource testRule &&
                        testRule.ifTrue() instanceof
            ...
            if (!additionalRules.contains(org)) additionalRules.add(org);
        }
    ...
    public static void injectNoiseBasedSurfaceRules(
        ...
        if (o instanceof SurfaceRuleProvider srp) {
            SurfaceRules.RuleSource originalRules =
                srp.wover_getOriginalSurfaceRules();
            srp.wover_overwriteSurfaceRules(
                mergeSurfaceRules(
                    dimensionKey,
                    originalRules,
                    loadedBiomeSource,
    ...
    static void injectSurfaceRulesToAllDimensions(
        ...
        for (var entry : dimensionRegistry.entrySet()) {
            ...
            if (
                stem.generator() instanceof InjectableSurfaceRules<?> generator
            ) {
                generator.wover_injectSurfaceRules(
                    dimensionRegistry,
                    dimensionKey
                );
            }
