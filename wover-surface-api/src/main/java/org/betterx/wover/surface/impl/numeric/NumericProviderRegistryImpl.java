...
public class NumericProviderRegistryImpl {
    ...
    public static final ResourceKey<
        MapCodec<? extends NumericProvider>
    > RANDOM_INT = NumericProviderRegistry.createKey(
        LibWoverSurface.C.id("rnd_int")
    );
    public static final ResourceKey<
        MapCodec<? extends NumericProvider>
    > NETHER_NOISE = NumericProviderRegistry.createKey(
        LibWoverSurface.C.id("nether_noise")
    );
