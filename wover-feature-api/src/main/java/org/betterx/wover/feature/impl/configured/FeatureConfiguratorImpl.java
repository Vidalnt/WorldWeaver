...
> {
    ...
    public static final EventImpl<
        OnBootstrapRegistry<ConfiguredFeature<?, ?>>
    > BOOTSTRAP_CONFIGURED_FEATURES = new EventImpl<>(
        "BOOTSTRAP_CONFIGURED_FEATURES"
    );
    ...
    protected final @Nullable BootstrapContext<
        ConfiguredFeature<?, ?>
    > bootstrapContext;
    ...
    public Holder<ConfiguredFeature<?, ?>> register() {
        if (key == null) {
            throw new IllegalStateException(
                "A ResourceKey can not be null if a feature should be registered!"
            );
        }
        if (bootstrapContext == null) {
            throwStateError(
                "Can not register a feature without a bootstrap context!"
            );
        }
        ConfiguredFeature<FC, F> cFeature = build();

        return bootstrapContext.register(key, cFeature);
    }
