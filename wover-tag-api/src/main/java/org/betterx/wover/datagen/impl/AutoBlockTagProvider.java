    ...
    implements WoverAutoProvider.WithRedirect
{
    ...
    private final List<
        WoverTagProvider<Block, TagBootstrapContext<Block>>
    ...
    public void prepareTags(TagBootstrapContext<Block> provider) {
        ...
        BuiltInRegistries.BLOCK.entrySet()
            .stream()
            .filter(entry ->
                modIDs.contains(entry.getKey().identifier().getNamespace())
            )
            .forEach(entry -> {
    ...
    private void addBlockTags(
        TagBootstrapContext<Block> provider,
    ...
    @Override
    public @Nullable <T extends DataProvider> WoverDataProvider<T> redirect(
        @Nullable WoverDataProvider<T> provider
    ) {
        if (provider instanceof WoverTagProvider<?, ?> tagProvider) {
            if (
                tagProvider.tagRegistry == this.tagRegistry &&
                tagProvider.modCore.equals(this.modCore)
            ) {
                LibWoverTag.C.LOG.debug(
                    "Redirecting {}  to {} ({})",
                ...
                this.mergeAllowedAndForced(
                    (WoverTagProvider<
                        Block,
                        TagBootstrapContext<Block>
                    >) tagProvider
                );
                redirects.add(
                    (WoverTagProvider<
